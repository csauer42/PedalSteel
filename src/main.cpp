...
int main()
{
    ...
    //  9    D3      148       50
    // 10    B2      123       47
    // * pitches shown equal tuning A = 440Hz for
    // reference. actual steel tuning uses sweetened E9.

    // TODO
    // - mutually exclusive pedal/lever check
    // - optional/optimal chord voicings for pedals/levers applied (maj/min/7th/etc)
    //   - standard picking groups by string:
    //     - 3/4/5
    //     - 4/5/6
    //     - 5/6/8
    //     - 6/8/10
    // - notes in scale degrees
    // - full fretboard output
    // - command line input / persistent interface
...
std::vector<Pedal> CreatePedals()
{
    ...
    // E: RKR 2(-1/-2), 9(-1)
    auto rkr = Pedal("E");
    rkr.changes.push_back({2, -2, true}); // half stop
